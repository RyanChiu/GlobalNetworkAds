...
$_sql_ = 
	...
	WHERE %s
		...
		AND agentid > 0 
	GROUP BY %s  ORDER BY `sales` desc  LIMIT 10;";
$sql = sprintf($_sql_, 0, $today, "convert(trxtime, date) >= '2016-08-14' and convert(trxtime, date) <= '$today'", 'agentid');
$rs = mysql_query("delete from top10s where flag = 0", $zconn->dblink)
	or die ("Something wrong with: " . mysql_error() . "\n");
$rs = mysql_query("insert into top10s " . $sql, $zconn->dblink)
	or die ("Something wrong with: " . mysql_error() . "\n");
$sql = sprintf($_sql_, 1, $today, "convert(trxtime, date) <= '$weekend' AND convert(trxtime, date) >= '$weekstart'", 'agentid');
$rs = mysql_query("delete from top10s where flag = 1", $zconn->dblink)
	or die ("Something wrong with: " . mysql_error() . "\n");
$rs = mysql_query("insert into top10s " . $sql, $zconn->dblink)
	or die ("Something wrong with: " . mysql_error() . "\n");
$sql = sprintf($_sql_, 2, $today, "convert(trxtime, date) <= '$monthend' AND convert(trxtime, date) >= '$monthstart'", 'agentid');
$rs = mysql_query("delete from top10s where flag = 2", $zconn->dblink)
	or die ("Something wrong with: " . mysql_error() . "\n");
$rs = mysql_query("insert into top10s " . $sql, $zconn->dblink)
	or die ("Something wrong with: " . mysql_error() . "\n");
$sql = sprintf($_sql_, 3, $today, "convert(trxtime, date) <= '$weekend' AND convert(trxtime, date) >= '$weekstart'", 'officename');
